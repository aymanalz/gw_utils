...
def add_simulated_multi_layer_heads(ax, row, col, layers, hds_obj, start_date, layer = None):
    """
    ...
    if not(layer is None):
        ax.plot(dates, head, label='Heads in Layer {}'.format(layer), linewidth=0.8)
    else:
        ax.plot(dates, head, label = 'Simulated Hydraulic Head', linewidth = 0.8)
    return dates,head

def plot_heads_piezometer(mfname, start_date, end_date, pdf_file = 'all_water_levels4.pdf',
                   add_water_table = True, x_limit = [], obs_name_file = None, piezometer = []):
    """
    ...
    allids = []
    for irecord, record in local_names.iterrows():
        allids.append(int(record['obsnme'].split('_')[1]))
        xx = 1
    local_names['ID'] = allids
    ...
    ilayer = 0
    ilayer_list = []
    for piezo in piezometer:
        piezo_ids.append(int(piezo.split("_")[1]))
        ilayer = ilayer + 1
        ilayer_list.append(ilayer)
    ...
    with PdfPages(pdf_file) as pdf:
        for well in well_groups:
            ...
            if not (well[0] in piezo_ids):
                continue

            # width=0.5,
            for iii, p_id in enumerate(piezo_ids):
                if well[0] == p_id:
                    ilayer = ilayer_list[iii]
                    break
            ...
            if 1:
                ssdate, sshead = add_simulated_multi_layer_heads(ax, row, col, layers, hds_obj, start_date, layer= ilayer)
            ...
            if 1:
                ax.scatter(dates, head1, marker=markers_list[loop_index], label = 'Observed Value  L{}'.format(ilayer), facecolors='none',
                           edgecolors= colors_list[0], s = 12, zorder=2, linewidths = 0.5)
                flg1 = 1
            else:
                ax.scatter(dates, head1, marker=markers_list[loop_index],  facecolors='none', edgecolors=colors_list[loop_index], s=12, zorder=2, linewidths = 0.1)
            ...
            if 1:
                ax.scatter(dates, head, marker=markers_list[loop_index], label='Simulated Equivalent L{}'.format(ilayer),  facecolors='none', edgecolors=colors_list[1],
                           s=12, linewidths = 0.5, zorder= 3)
                flg = 1
            else:
                ax.scatter(dates, head, marker=markers_list[loop_index],  facecolors='none', edgecolors='b', s=12, linewidths = 0.1,  zorder= 3)
            ...
            try:
                massk = np.logical_and(dates>=x_limit[0], dates<=x_limit[1]-2)
                maxV = max(head[massk])
                minV = min(head[massk])
                massk2 = np.logical_and(ssdate >= x_limit[0], ssdate <= x_limit[1] - 2)
                maxVs = max(sshead[massk2])
                minVs = min(sshead[massk2])
                #mmean = np.nanmean((head[massk]))
            except:
            ...
            try:
                ...
                xx = 1
            plt.ylim([minV, maxV])
            locName = local_names.loc[local_names['ID']==well[0], 'shortNam']
            try:
                tit = "Well ID:{}" \
